...
def loginactionteacher(request):
    global id, password, FEmailid1, id1,tid
    if request.method=="POST":
        m = sql.connect(user="root",password="adarshkunal", host="localhost", database="MentorSlotBooking", auth_plugin='mysql_native_password')
        cursor=m.cursor()
        d=request.POST
        ...
        if t1 ==():
            ...
            return render(request, "AC_login_interface.html", data1)
...
def AChomeaction(request):
        ...
        return render(request, "AC_Home.html", data1)



def group_details_ac(request):
    if request.method=="GET":
        m = sql.connect(user="root",password="adarshkunal", host="localhost", database="mentorslotbooking", auth_plugin='mysql_native_password')
        cursor=m.cursor()
        d=request.POST
        
        c = "select * from group_details where teacher_id='{}'".format(tid)
        cursor.execute(c)
        t1=tuple(cursor.fetchall()) 
        ...
        return render(request, "group_details.html", data)
    

def group_marks_ac(request):
    if request.method=="GET":
        m = sql.connect(user="root",password="adarshkunal", host="localhost", database="mentorslotbooking", auth_plugin='mysql_native_password')
        cursor=m.cursor()
        d=request.POST
        
        c = "select * from group_marks_ac where teacher_id='{}'".format(tid)
        cursor.execute(c)
        t1=tuple(cursor.fetchall()) 
        ...
        return render(request, "marks_detail_ac.html", data)

# panel_members_details_ac
def panel_members_details_ac(request):
    if request.method=="GET":
        m = sql.connect(user="root",password="adarshkunal", host="localhost", database="mentorslotbooking", auth_plugin='mysql_native_password')
        cursor=m.cursor()
        d=request.POST       
        return render(request, "AC_panel_member_details.html")
